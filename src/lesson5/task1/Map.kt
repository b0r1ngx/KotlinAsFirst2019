...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    val toGrade = mutableMapOf<Int, MutableList<String>>()
    for ((name, grade) in grades) {
        if (toGrade[grade] == null) {
            toGrade[grade] = mutableListOf(name)
        } else {
            toGrade[grade]!!.add(name)
        }
    }
    return toGrade
}
...
fun hasAnagrams(words: List<String>): Boolean {
    val a = mutableSetOf<Set<Char>>()
    for (i in words) a.add(i.toLowerCase().toSet())
    return (a.size != words.size)
}
// Выше преведено решение через множества
